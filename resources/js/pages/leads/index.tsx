...
import { Archive, Columns3, LayoutGrid, List, Plus, Search } from 'lucide-react';
...
export default function LeadsIndex({ leads, owners, statuses, sources, kanban, filters, can }: Props) {
    ...
    return <AppLayout>
        <Head title="Leads" />
        <PageHeader title="Leads" badge={<Badge tone="neutral" size="sm">{leads.total}</Badge>} actions={<div className="flex items-center gap-2"><Link href="/leads/archived"><Button variant="secondary"><Archive /> Archived</Button></Link>{can.create && <Link href="/leads/create"><Button><Plus /> New lead</Button></Link>}</div>} />
