...
function ImportCard({ title, endpoint, icon: Icon, can }: { title: string; endpoint: string; icon: typeof ContactRound; can: boolean }) {
    ...
    if (!can) return null;
    return <Card><CardHeader title={title} /><CardBody className="space-y-3"><div className="flex items-center gap-3"><span className="flex size-9 items-center justify-center rounded-lg bg-brand-wash text-brand"><Icon className="size-4" /></span><div className="min-w-0"><p className="text-xs font-medium text-ink">Choose a CSV file</p><p className="truncate text-2xs text-ink-3">Required columns are validated before any records are created.</p></div></div><input ref={input} type="file" accept=".csv,text/csv" className="sr-only" onChange={(event) => setFile(event.target.files?.[0] ?? null)} /><div className="flex flex-wrap items-center gap-2"><button type="button" onClick={() => input.current?.click()} className={cn(buttonVariants({ variant: 'secondary', size: 'sm' }))}><FileUp className="size-3.5" /> {file ? file.name : 'Choose file'}</button>{file && <button type="button" onClick={submit} disabled={busy} className={cn(buttonVariants({ variant: 'primary', size: 'sm' }))}>{busy ? 'Importing…' : 'Import records'}</button>}<a href={`${endpoint.replace('/imports/', '/imports/templates/')}`} className="text-2xs font-medium text-brand hover:underline">Download template</a></div></CardBody></Card>;
}
