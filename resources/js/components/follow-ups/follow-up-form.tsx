...
type EntityType = 'lead' | 'customer' | 'deal';

function localDateTime(value: string | null | undefined): string { return value ? value.slice(0, 16) : ''; }

export function FollowUpForm({ followUp, leads, customers, deals, owners, action, method, submitLabel }: { followUp?: FollowUp; leads: EntityOption[]; customers: EntityOption[]; deals: DealOption[]; owners: UserRef[]; action: string; method: 'post' | 'put'; submitLabel: string }) {
    const initialType: EntityType = followUp?.deal_id ? 'deal' : followUp?.customer_id ? 'customer' : 'lead';
    ...
    const entityOptions = entityType === 'lead'
        ...
        : entityType === 'customer'
            ...
            : deals.map((item) => ({ value: String(item.id), label: item.title, hint: item.customer_id ? 'Customer deal' : 'Lead deal' }));
    const entityValue = entityType === 'lead' ? data.lead_id : entityType === 'customer' ? data.customer_id : data.deal_id;
    ...
    function selectEntity(type: EntityType) {
        setEntityType(type);
        setData({ ...data, lead_id: type === 'lead' ? data.lead_id : '', customer_id: type === 'customer' ? data.customer_id : '', deal_id: type === 'deal' ? data.deal_id : '' });
    }
