...
export function LeadSettingsPage({ kind, items, filters }: { kind: 'statuses' | 'sources'; items: Paginated<ConfigItem>; filters: { search: string; status: string; per_page?: number } }) {
    ...
    const [statusConfirming, setStatusConfirming] = useState<ConfigItem | null>(null);
    ...
    function changeStatus() {
        ...
        router.patch(`${endpoint}/${statusConfirming.id}/toggle`, {}, { preserveScroll: true, onSuccess: () => setStatusConfirming(null) });
    }
    ...
    return <AppLayout>
        ...
        <Modal open={Boolean(statusConfirming)} onClose={() => setStatusConfirming(null)} title={`${statusConfirming?.status === 'active' ? 'Deactivate' : 'Activate'} ${singular}`} width="sm" footer={<><Button type="button" variant="secondary" onClick={() => setStatusConfirming(null)}>Cancel</Button><Button type="button" variant={statusConfirming?.status === 'active' ? 'danger' : 'primary'} onClick={changeStatus}>{statusConfirming?.status === 'active' ? 'Deactivate' : 'Activate'} {singular}</Button></>}>
            <p className="text-sm text-ink-2">Are you sure you want to {statusConfirming?.status === 'active' ? 'deactivate' : 'activate'} <span className="font-semibold text-ink">{statusConfirming?.name}</span>?</p>
            {statusConfirming?.status === 'active' && <p className="mt-3 rounded-md bg-warn-wash px-3 py-2 text-xs text-warn">Inactive options cannot be selected for new leads until they are activated again.</p>}
        </Modal>
    </AppLayout>;
}
